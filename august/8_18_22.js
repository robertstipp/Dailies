...
function setup() {
  ...
  for (let radius = 500; radius > 10; radius -= 3) {
    rings.push(new Ring(radius));
  }
  ...
  rings.forEach((ring) => {
    ...
    let angleStep = map(radius, 10, 500, 0.05, 0.001);
    let elSize = map(radius, 10, 500, 5, 10);

    beginShape();
    stroke(c);
    for (let angle = 0; angle < TAU; angle += angleStep) {
      let x = 300 + radius * Math.cos(angle);
      let y = 300 + radius * Math.sin(angle);

      let xOff =
        openSimplex.noise2D(x / 100, y / 100) * map(radius, 10, 500, 1, 100);
      let yOff =
        openSimplex.noise2D(x / 100, y / 100) * map(radius, 10, 500, 1, 100);
      ...
      let colorIndex = Math.floor(
        map(openSimplex.noise2D(x / 100, y / 100), -1, 1, 0, colors.length - 1)
      );
...
function draw() {
  // for (let radius = 500; radius > 10; radius -= 100) {
  //   fill(color(colors[0]));
  //   beginShape();
  ...
  //     let y = 300 + radius * Math.sin(angle);
  //     let xOff =
  //       openSimplex.noise3D(x / 100, y / 100, frameCount / 100) *
  //       map(radius, 10, 200, 1, 100);
  //     let yOff =
  //       openSimplex.noise3D(x / 100, y / 100, frameCount / 100) *
  //       map(radius, 10, 200, 1, 100);
  //     x += xOff;
...
class Ring {
  constructor(r) {
    ...
    let colorIndex = Math.floor(map(r, 10, 500, 0, colors.length - 1));
